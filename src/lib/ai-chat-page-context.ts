...
Overview (for other questions only): ${study.overviewText}`;
  ...
  if (pagePath.startsWith("/notes/")) {
    return `The user is reading JB's Case Notes at ${pagePath}.
JB's Case Notes #1 is JB's working note on Cisco Policy Copilot: why dumping a policy draft into chat fails, and why a side-by-side workspace (intent on the left, draft on the right) works better.
