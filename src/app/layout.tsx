...
export default function RootLayout({
  ...
  return (
    <html lang="en" className={`${ibmPlexMono.variable} bg-background`}>
      <head>
        <link rel="preconnect" href={BLOB_CDN_ORIGIN} crossOrigin="anonymous" />
        <JsonLd data={[personJsonLd(), webSiteJsonLd()]} />
      </head>
      <body className={`${ibmPlexMono.className} relative min-h-screen bg-background text-white antialiased`}>
        <WireframeFilters />
        <SkipToContentLink />
        <CacheManager />
        <ScrollToTop />
