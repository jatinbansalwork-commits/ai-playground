      ...
      <ol className="flex max-w-md flex-col">
        {steps.map((step, index) => (
          <li key={step} className="flex w-full flex-col">
            <span className="w-full rounded-lg border border-[#FF894A]/35 bg-[#FF894A]/10 px-4 py-3 text-left text-[15px] leading-snug text-white">
              {step}
            </span>
            {index < steps.length - 1 ? (
              <span
                className="py-2 pl-4 text-lg leading-none text-[#FFC799]"
                aria-hidden
              >
                ↓
              </span>
