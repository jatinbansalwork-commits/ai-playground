        ...
        <thead>
          <tr style={{ backgroundColor: CLAUDE.surfaceRaised }}>
            {["Term", "Resolved to", "Type"].map((h) => (
              <th
                key={h}
                className={cn("font-medium", compact ? "px-2 py-1.5 text-[8px]" : "px-2.5 py-2 text-[9px]")}
        ...
        <tbody>
          {EHR_ENTITY_MAPPINGS.map((row) => (
            <tr key={row.term} style={{ borderTop: `1px solid ${CLAUDE.hairline}` }}>
              <td className={cn(compact ? "px-2 py-1.5 text-[9px]" : "px-2.5 py-2 text-[10px]")} style={{ color: CLAUDE.text }}>
                {row.term}
              </td>
              <td className={cn(compact ? "px-2 py-1.5 text-[9px]" : "px-2.5 py-2 text-[10px]")} style={{ color: CLAUDE.primary }}>
                {row.resolved}
              </td>
              <td className={cn(compact ? "px-2 py-1.5 text-[8px]" : "px-2.5 py-2 text-[9px]")} style={{ color: CLAUDE.textMuted }}>
                {row.type}
              </td>
            </tr>
          ))}
...
export function CandidateRuleDualView({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="space-y-2">
      <div
        className="rounded-xl border p-2.5 font-mono text-[9px] leading-relaxed md:text-[10px]"
      ...
      >
        ...
        <p className={cn("mt-1.5 leading-relaxed", compact ? "text-[10px]" : "text-[11px]")} style={{ color: CLAUDE.textSecondary }}>
          {EHR_RULE_REASONING}
        </p>
      </motion.div>
    </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      <div className="rounded-xl border p-2.5" style={{ borderColor: CLAUDE.border, backgroundColor: CLAUDE.surface }}>
        <p className="text-[9px]" style={{ color: CLAUDE.textMuted }}>Who&apos;s affected</p>
        <p className="mt-1 text-[10px] leading-snug" style={{ color: CLAUDE.textSecondary }}>
          {EHR_UNIT_TEST_RESULTS.blastRadius}
        </p>
      </div>
      <div className="rounded-xl border p-2.5" style={{ borderColor: CLAUDE.border, backgroundColor: CLAUDE.surface }}>
        <p className="text-[9px]" style={{ color: CLAUDE.textMuted }}>Risk score</p>
        <p className="mt-1 text-[14px] font-medium tabular-nums" style={{ color: scoreColor }}>
          {EHR_UNIT_TEST_RESULTS.riskScore}
        </p>
        <p className="text-[9px]" style={{ color: CLAUDE.textSoft }}>{EHR_UNIT_TEST_RESULTS.riskDetail}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <ul className="space-y-1.5">
      {[
        ...
        <li key={item} className="flex items-start gap-2 text-[10px]" style={{ color: CLAUDE.textSecondary }}>
    ...
    <div className="space-y-2">
      {EHR_IMPACT_FORECAST.map((item) => (
        <div key={item.label} className="rounded-lg border px-2.5 py-2" style={{ borderColor: CLAUDE.border, backgroundColor: CLAUDE.surface }}>
          <p className="text-[9px] font-medium" style={{ color: CLAUDE.textMuted }}>{item.label}</p>
          <p className="mt-0.5 text-[10px] leading-snug" style={{ color: CLAUDE.textSecondary }}>{item.value}</p>
        </div>
      ))}
      <div className="rounded-lg border p-2.5" style={{ borderColor: CLAUDE.primaryBorder, backgroundColor: CLAUDE.primaryMuted }}>
        <p className="text-[9px] font-medium" style={{ color: CLAUDE.primary }}>Before you go live</p>
        <ul className="mt-1 space-y-0.5">
          {EHR_MITIGATION_SUGGESTIONS.map((s) => (
            <li key={s} className="text-[9px]" style={{ color: CLAUDE.textSecondary }}>· {s}</li>
          ))}
        </ul>
      </div>
    </div>
      ...
      {EHR_COMPLIANCE_OVERLAYS.map((item, i) => {
        ...
        return (
          <div
            ...
            style={{ borderColor: CLAUDE.border, backgroundColor: CLAUDE.surface }}
          >
            <p className="text-[9px] font-medium" style={{ color: CLAUDE.text }}>{item.framework}</p>
            <p className="mt-0.5 text-[8px] leading-snug" style={{ color: CLAUDE.textSoft }}>{item.control}</p>
            <p
...
export function DriftMemoryCard({
  ...
  return (
    <div className="rounded-xl border p-3" style={{ borderColor: CLAUDE.warningMuted, backgroundColor: CLAUDE.surface }}>
      <p className="text-[10px] font-medium" style={{ color: CLAUDE.warning }}>
        Unused rule — {LINKEDIN_DRIFT_MEMORY.rule}
      </p>
      <p className="mt-1.5 text-[10px] leading-relaxed" style={{ color: CLAUDE.textSecondary }}>
        {LINKEDIN_DRIFT_MEMORY.why}
      </p>
      <p className="mt-1 text-[9px]" style={{ color: CLAUDE.textMuted }}>{LINKEDIN_DRIFT_MEMORY.simulation}</p>
      <p className="mt-1 text-[9px] italic" style={{ color: CLAUDE.warning }}>{LINKEDIN_DRIFT_MEMORY.flag}</p>
      {!resolved ? (
        ...
        <p className="mt-2 text-[9px] font-medium" style={{ color: CLAUDE.validated }}>Resolved — record updated</p>
      )}
    </div>
  );
}
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline justify-between gap-2">
              <p className="text-[11px] font-medium" style={{ color: CLAUDE.text }}>{event.stage}</p>
              <p className="shrink-0 text-[9px] tabular-nums" style={{ color: CLAUDE.textSoft }}>{event.time}</p>
            </div>
            <p className="mt-0.5 text-[10px] leading-relaxed" style={{ color: CLAUDE.textMuted }}>{event.evidence}</p>
          </div>
        </motion.div>
      ))}
    </div>
...
export function L7UpgradeBadge() {
  return (
    <div className="rounded-lg border px-2.5 py-2" style={{ borderColor: CLAUDE.validatedMuted, backgroundColor: CLAUDE.validatedMuted }}>
      <p className="text-[10px] font-medium" style={{ color: CLAUDE.validated }}>{EHR_L7_UPGRADE.title}</p>
      <p className="mt-0.5 text-[9px]" style={{ color: CLAUDE.textSecondary }}>{EHR_L7_UPGRADE.detail}</p>
    </div>
  );
}
