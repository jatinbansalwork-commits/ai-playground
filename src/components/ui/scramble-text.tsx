...
import { useState, useEffect, useRef, useLayoutEffect, Fragment, type CSSProperties } from "react";
export type ScrambleTextEnterMode = "none" | "oneLine" | "multiLine" | "random";
...
export interface ScrambleTextProps {
  ...
  enterAnimation?: {
    mode?: ScrambleTextEnterMode;
    ease?: { type?: string; duration?: number; ease?: string | number[] };
    scrambleIntensity?: number;
    replay?: boolean;
    position?: "above" | "middle" | "below";
    restState?: "solid" | "empty";
  ...
  hoverAnimation?: {
    ...
    flickerIntensity?: number;
    flickerSpeed?: number;
    waveEase?: { type?: string; duration?: number; ease?: string | number[] };
...
export function ScrambleText(props: ScrambleTextProps) {
    const { words, enterAnimation, hoverAnimation, color, font, tag, style } =
        props
    ...
    // ── Line detection ────────────────────────────────────────────────────────
    const lineGroupsRef = useRef<number[][]>([])
    const detectLines = () => {
        ...
        // Guard against redundant updates: setLineGroups always creates a new
        // array, and the enter-animation effect depends on [lineGroups] and
        // restarts (wiping displays) whenever that reference changes. Several
        // measurement passes (mount, rAF, fonts.ready, resize) can fire in
        // quick succession with an identical result — only commit when the
        // grouping actually changed, so the reveal isn't repeatedly cancelled
        // mid-flight.
        const prev = lineGroupsRef.current
        const same =
            prev.length === allLines.length &&
            prev.every(
                (g, i) =>
                    g.length === allLines[i].length &&
                    g.every((wi, j) => wi === allLines[i][j])
            )
        if (same) return
        lineGroupsRef.current = allLines
    ...
    useEffect(() => {
        ...
        obs.observe(el)
        // Content-hugging (max-content/nowrap) layout means the container's
        // own box can be stable at mount even if the ghost measuring spans
        // haven't reflowed to their final font metrics yet (webfont swap,
        // late layout pass). ResizeObserver alone can miss that correction
        // since there may be no further resize to react to. Re-measure once
        // fonts are confirmed ready and once more on the next frame so a
        // stale/partial first read of lineGroups can't get stuck.
        let rafId = requestAnimationFrame(() => detectLines())
        if (typeof document !== "undefined" && (document as any).fonts?.ready) {
            ;(document as any).fonts.ready.then(() => detectLines())
        }
        return () => {
            obs.disconnect()
    ...
    return (
        <div
            ...
            style={{
                ...
                ...(isFixedWidth
                    ? { width: "100%" }
                    : { minWidth: "max-content", width: "auto" }),
            }}
        >
            {paragraphs.map((_, pi) => {
                const paraEntries = allWords.filter((w) => w.pi === pi)
